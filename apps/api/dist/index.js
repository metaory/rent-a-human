...
// src/index.ts
var AGENTS_LIST_KEY = "agent:ids";
var avatar = /* @__PURE__ */ __name((seed) => `https://api.dicebear.com/9.x/notionists/svg?seed=${encodeURIComponent(seed)}`, "avatar");
var defaultAgents = [
  {
    id: "agt-001",
    name: "Marcus Cole",
...
async function getAgentsList(kv) {
  const raw2 = await kv.get(AGENTS_LIST_KEY);
  if (!raw2)
    return [];
  const ids = JSON.parse(raw2);
  const out = [];
  for (const id of ids) {
...
__name(getAgentsList, "getAgentsList");
function agentFromBody(body, id) {
...
__name(agentFromBody, "agentFromBody");
var app = new Hono2();
app.use(
  "*",
  cors({
...
app.get("/agents", async (c) => {
  ...
  if (list.length === 0) {
    await kv.put(AGENTS_LIST_KEY, JSON.stringify(defaultAgents.map((a) => a.id)));
...
app.get("/agents/:id", async (c) => {
  ...
  const raw2 = await c.env.AGENTS.get(`agent:${id}`);
  if (!raw2)
